...
for(sn in c(1,3,5)){
	...
	knots <- seq(from=0, to=1, by=0.2)
	b.basis <- bsplineS(obs.domain, knots)
	b.expand <- bsplineS(domain, knots)
	simul.norm <- matrix(nrow=length(obs.domain), ncol=200)
	true1 <- 2-cos(pi*obs.domain)
	true2 <- 3*obs.domain
	true.cluster <- sample(c(1,2,3), size=N, rep=T, prob=c(0.33, 0.34, 0.33))
	poo <- function(s){return(ral(1, sigma=0.2, tau=s)-0.2*(1/s-1/(1-s)))}
	for(i in 1:N){
		if(true.cluster[i]==1){
			simul.norm[,i] <- true1+sapply(rep(0.1, length=101), poo)
		}else if(true.cluster[i]==2){
			simul.norm[,i] <- true1+sapply(rep(0.9, length=101), poo)
		}else{
			simul.norm[,i] <- true2+sapply(rep(0.9, length=101), poo)
		}
	}
   ...
   J <- 5
	...
	#2-9)curveclust
	   simul.norm.curv <- matrix(nrow=128, ncol=300)
...
for(sn in c(1,3,5)){
	...
	for(i in 1:N){
		if(true.cluster[i]==1){
			...
			simul.norm[,i] <- true2+sapply(rep(0.7, length=101), poo)
		}
	}
...
for(sn in c(1,3,5)){
	...
	for(i in 1:N){
		if(true.cluster[i]==1){
			...
			simul.norm[,i] <- true2+sapply(rev(tau.vec), poo)
		}
